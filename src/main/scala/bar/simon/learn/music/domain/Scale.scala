...
object Scale {
  ...
  def minor(note: Note): Scale = Scale(note, "1 2 b3 4 5 b6 b7")

  def majorPentatonic(note: Note): Scale = Scale(note, "1 3 4 5 7")
  def minorPentatonic(note: Note): Scale = Scale(note, "1 b3 4 5 b7")
}